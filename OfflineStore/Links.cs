...
namespace Baconography.OfflineStore
{
    class Links
    {
        ...
        private async Task<string> TranslateSubredditNameToId(string subredditName)
        {
            var getit = new GetSubreddit { Name = subredditName };
            return (await getit.Run()).Data.Name;
        }

        public async Task<Listing> LinksForSubreddit(string subredditName, string after)
        {
            var subredditId = await TranslateSubredditNameToId(subredditName);
